...
## Running PACER Locally

PACER runs as a Docker Compose stack (web, worker, MongoDB, Redis, and MinIO). Configuration is read from the `.env` file in the repository root. Requires Docker with the Compose plugin.
...
Mounts `pacer_web` and `pacer_worker` into the containers and runs them in watch mode so code changes reload automatically (webpack watch for the web, `watchmedo` for the worker):
...
The PACER docker images are published to the [GitHub Container Registry (GHCR)](https://ghcr.io) under the `natlabrockies` org for easy deployment via Helm or other docker services. The images include:

- [PACER Web](https://github.com/orgs/NatLabRockies/packages/container/package/pacer-webservice%2Fweb) — `ghcr.io/natlabrockies/pacer-webservice/web`
- [PACER Worker](https://github.com/orgs/NatLabRockies/packages/container/package/pacer-webservice%2Fworker) — `ghcr.io/natlabrockies/pacer-webservice/worker`
- [PACER Grafana](https://github.com/orgs/NatLabRockies/packages/container/package/pacer-webservice%2Fgrafana) — `ghcr.io/natlabrockies/pacer-webservice/grafana`

Pull an image with:

```bash
docker pull ghcr.io/natlabrockies/pacer-webservice/web:latest
```
