...
def test_run_file_conveyance(dispatcher: Dispatcher):
    ...
    response = send_message_and_wait(
        file_io_job,
        "write",
        {"file_name": test_file_name, "contents": test_file_contents},
    )
    ...
    response = send_message_and_wait(
        file_io_job, "read", {"file_name": "non-existant-file.io"}
    )
...
def test_checkin_on_run_error(dispatcher: Dispatcher):
    ...
    send_message_and_wait(
        error_mock_job, "throw_error_with_value", {"value": test_string}
    )

    wait_for_run_status(run, RunStatus.ERROR)

    log_reader_job = dispatcher.create_job(
        LogReaderJob.job_path(), {"run_id": run.ref_id}
    )
