...
from pacer_worker.lib.constants import DATETIME_FORMAT
...
from pacer_worker.lib.utils import exc_to_str
...
class StepRunProcess(StepRunBase):
    ...
    def initialize_simulation(self) -> None:
        ...
        self.simulation_process = Process(
            target=StepRunProcess._start_simulation_process, args=(self,)
        )
        self.simulation_process.start()

        self._wait_for_event(
            self.running_event, self.options.start_timeout, desired_event_set=True
        )
    ...
    def check_for_errors(self) -> None:
        ...
        if exit_code:
            raise JobExceptionExternalProcess(
                f"Simulation process exited with non-zero exit code: {exit_code}"
            )

    def _wait_for_event(
        self, event: threading.Event, timeout: float, desired_event_set: bool = False
    ) -> None:
        """Wait for a given event to go be set or cleared within a given amount of time"""
        wait_until = time() + timeout
        while (
            event.is_set() != desired_event_set
            and time() < wait_until
        ...
        if not self.simulation_process.is_alive():
            self.check_for_errors()
            raise JobExceptionExternalProcess(
                "Simulation process exited without returning an error"
            )
        if time() > wait_until:
            self.simulation_process.kill()
            raise TimeoutError(
                "Timedout waiting for simulation process to toggle event"
            )
    ...
    def advance(self) -> None:
        ...
        if self.advance_event.is_set():
            raise JobExceptionMessageHandler(
                "Cannot advance, simulation is already advancing"
            )
        self.advance_event.set()
        self._wait_for_event(
            self.advance_event,
            timeout=self.options.advance_timeout,
            desired_event_set=False,
        )
        self.update_run_time()
    ...
    def stop(self):
        if not self.stop_event.is_set():
            ...
            self.stop_event.set()
            while (
                self.simulation_process.is_alive()
                and time() - stop_start_time < self.options.stop_timeout
                and not self.error_event.is_set()
            ):
                pass
            if (
                time() - stop_start_time > self.options.stop_timeout
                and self.simulation_process.is_alive()
            ):
                self.simulation_process.kill()
                raise JobExceptionExternalProcess(
                    "Simulation process stopped responding and was killed."
                )
            if self.error_event.is_set():
                self.handle_process_error()
