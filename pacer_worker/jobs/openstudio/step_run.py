...
from pacer_worker.jobs.openstudio.lib.openstudio_component import (
    OpenStudioComponent
)
from pacer_worker.jobs.openstudio.lib.openstudio_point import OpenStudioPoint
from pacer_worker.jobs.step_run_process import StepRunProcess
from pacer_worker.lib.job_exception import (
    JobException,
...
class StepRun(StepRunProcess):
    def __init__(
        ...
        super().__init__(
            run_id, realtime, timescale, external_clock, start_datetime, end_datetime
        )
        ...
        for alfalfa_json in self.run.dir.glob("**/run/alfalfa.json"):
            self.ep_points += [
                OpenStudioPoint(**point) for point in json.load(alfalfa_json.open())
            ]

        def add_additional_meter(
            fuel: str, units: str, converter: Callable[[float], float]
        ):
            meter_component = OpenStudioComponent(
                "Meter", {"meter_name": f"{fuel}:Building"}, converter
            )
            meter_point = OpenStudioPoint(
                id=f"whole_building_{fuel.lower()}",
                name=f"Whole Building {fuel}",
                units=units,
            )
            meter_point.output = meter_component
            self.ep_points.append(meter_point)

        add_additional_meter(
            "Electricity", "W", lambda x: x / self.options.timesteps_per_hour
        )
        add_additional_meter(
            "NaturalGas", "W", lambda x: x / self.options.timesteps_per_hour
        )
    ...
    def simulation_process_entrypoint(self):
        ...
        self.ep_api.runtime.callback_begin_new_environment(
            self.ep_state, self.initialize_handles
        )
        self.ep_api.runtime.callback_end_zone_timestep_after_zone_reporting(
            self.ep_state, self.ep_begin_timestep
        )
        ...
        return_code = self.ep_api.runtime.run_energyplus(
            state=self.ep_state,
            command_line_args=[
                "-w",
                str(self.weather_file),
                "-d",
                str(self.dir / "simulation"),
                "-r",
                str(self.idf_file),
            ],
        )
        ...
        if return_code != 0:
            ...
            raise JobExceptionExternalProcess(
                f"EnergyPlus Exited with a non-zero exit code: {return_code}"
            )
    ...
    def initialize_handles(self, state):
        """Callback called at begin_new_environment. Enumerates PACER points to connect
        ...
        if len(exceptions) > 0:
            ExceptionGroup(
                "Exceptions generated while initializing EP handles", exceptions
            )
        self.run.save()
    ...
    def get_sim_time(self) -> datetime:
        sim_time = self.options.start_datetime.replace(hour=0, minute=0) + timedelta(
            hours=self.ep_api.exchange.current_sim_time(self.ep_state)
        )
        sim_time -= timedelta(
            minutes=1
        )  # Energyplus gives time at the end of current timestep, we want the beginning
        return sim_time
    ...
    def ep_read_outputs(self):
        ...
        for point in self.ep_points:
            ...
            if (
                self.options.historian_enabled
        ...
        if self.historian_enabled:
            try:
                response = self.influx_client.write_points(
            ...
            if not response:
                self.logger.warning(
                    f"Unsuccessful write to influx.  Response: {response}"
                )
    ...
    def prepare_idf(self):
        ...
        delete_objects = [
            *workspace.getObjectsByType(openstudio.IddObjectType("RunPeriod")),
        ...
        runperiod_object.setString(0, "PACER Run Period")
        ...
        paths_objects = workspace.getObjectsByType(
            openstudio.IddObjectType("PythonPlugin:SearchPaths")
        )
        python_paths = openstudio.IdfObject(
            openstudio.IddObjectType("PythonPlugin:SearchPaths")
        )
        python_paths.setString(0, "PACER Virtual Environment Path")
        ...
        if (self.run.dir / ".venv").exists():
            python_paths.setString(
                n, str(self.run.dir / ".venv" / "lib" / "python3.12" / "site-packages")
            )
            n += 1
