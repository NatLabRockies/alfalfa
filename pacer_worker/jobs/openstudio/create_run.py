...
from pacer_worker.jobs.openstudio import lib_dir
from pacer_worker.lib.enums import RunStatus, SimType
from pacer_worker.lib.job import Job
from pacer_worker.lib.job_exception import JobExceptionInvalidModel
from pacer_worker.lib.utils import rel_symlink
...
class CreateRun(Job):
    ...
    def exec(self):
        ...
        self.log_subprocess(
            [
                "openstudio",
                str(lib_dir / "merge_osws.rb"),
                str(default_workflow_path),
                str(submitted_osw_path),
            ]
        )
        ...
        metadata_json_path = (
            submitted_workflow_path / "reports" / "pacer_metadata_report_metadata.json"
        )
        if metadata_json_path.exists():
            self.load_metadata(metadata_json_path)
        else:
            self.logger.info("Could not find PACER Metadata Report")
    ...
    def validate(self) -> None:
        ...
        assert (self.dir / "simulation" / "sim.epw").exists(), (
            "Weather file was not created"
        )
