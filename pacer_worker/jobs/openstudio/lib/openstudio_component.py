...
from pacer_worker.lib.job_exception import JobException
...
class OpenStudioComponent:
    ...
    def __init__(
        ...
        parameters: dict,
        handle: int = None,
        converter: Callable[[float], float] = None,
    ):
    ...
    def initialize(self, api, state):
        if self.type == OpenStudioComponentType.GLOBAL_VARIABLE:
            self.handle = api.exchange.get_ems_global_handle(
                state, var_name=self.parameters["variable_name"]
            )
            ...
            self.handle = api.exchange.get_actuator_handle(
                state,
        ...
        if self.handle == -1:
            raise JobException(
                f"Handle not found for point of type: {self.type} and parameters: {self.parameters}"
            )
