...
from pacer_worker.lib.enums import AutoName, RunStatus
from pacer_worker.lib.job import Job, message
from pacer_worker.lib.job_exception import JobException, JobExceptionSimulation
from pacer_worker.lib.pacer_connections_manager import PACERConnectionsManager
from pacer_worker.lib.utils import to_bool
...
class StepRunBase(Job):
    ...
    def exec(self) -> None:
        ...
        if (
            not self.warmup_cleared
            and self.options.clock_source == ClockSource.INTERNAL
        ):
            self.advance()
    ...
    def run_timescale(self):
        ...
        while self.is_running:
            if datetime.now() >= next_advance_time:
                steps_behind = (
                    datetime.now() - next_advance_time
                ) / self.options.advance_interval
                if steps_behind > self.options.timescale_lag_limit:
                    raise JobExceptionSimulation(
                        f"Timescale too high. Simulation more than {self.options.timescale_lag_limit} timesteps behind"
                    )
                next_advance_time = next_advance_time + self.options.advance_interval

                self.logger.debug(
                    f"Internal clock called advance at {self.run.sim_time}"
                )
                self.logger.debug(f"Next advance time: {next_advance_time}")
                self.advance()

            if (
                self.check_simulation_stop_conditions()
                or self.run.sim_time >= self.options.end_datetime
            ):
