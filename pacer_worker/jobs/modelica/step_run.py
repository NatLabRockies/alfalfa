...
from pacer_worker.lib.enums import PointType
from pacer_worker.lib.job import message
from pacer_worker.lib.models import Point
from pacer_worker.lib.testcase import TestCase
...
class StepRun(StepRunBase):
    def __init__(
        ...
        self.logger.info(
            f"current datetime at start of simulation: {self.options.start_datetime}"
        )
    ...
    def initialize_simulation(self) -> None:
        ...
        config = {
            "fmupath": fmupath,
            "start_time": (
                self.options.start_datetime
                - datetime(self.options.start_datetime.year, 1, 1)
            )
            / timedelta(seconds=1),
    ...
    def get_sim_time(self) -> datetime:
        sim_time = datetime(
            self.options.start_datetime.year, 1, 1, 0, 0, 0
        ) + timedelta(seconds=float(self.tc.final_time))
    ...
    def setup_points(self):
        ...
        for output in output_names:
            point = Point(
                ref_id=name_to_id(output), name=output, point_type=PointType.OUTPUT
            )
            self.run.add_point(point)
                ...
                influx_points.append(
                    {
                        ...
                        "tags": {"id": point.ref_id, "point": True, "source": "pacer"},
