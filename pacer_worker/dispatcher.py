...
# pacer connections. WorkerJobBase could/should be updated to inherit
# from a new class that just handles the pacer connections, then
# the WorkerJobBase and Dispatcher can both inherit from the new class.
from pacer_worker.lib.job import Job, JobStatus
from pacer_worker.lib.logger_mixins import DispatcherLoggerMixin
from pacer_worker.lib.pacer_connections_manager import PACERConnectionsManager
from pacer_worker.lib.run_manager import RunManager


class Dispatcher(DispatcherLoggerMixin):
    """Class to pop data off a queue and determine to where the work needs
    to go. PACER works off a single queue and the work is identified
    ...
    def process_message(self, message):
        ...
        try:
            ...
            self.logger.error(
                "Exception while processing message: {} with {}".format(e, tb)
            )
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                tb = traceback.format_exc()
                self.logger.info(
                    "Exception caught in dispatcher.run: {} with {}".format(e, tb)
                )
