...
from pacer_worker.lib.singleton import Singleton


class PACERConnectionsManager(metaclass=Singleton):
    ...
    def _wait_for_service(
        self, service_name, check_function, max_retries=30, retry_delay=2
    ):
        """
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(
                    f"Attempting to connect to {service_name} (attempt {attempt + 1}/{max_retries})"
                )
                if check_function():
            ...
            if attempt < max_retries - 1:
                self.logger.info(
                    f"Waiting {retry_delay} seconds before retrying {service_name}"
                )
