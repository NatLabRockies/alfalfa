...
from pacer_worker.lib.enums import SimType
from pacer_worker.lib.logger_mixins import LoggerMixinBase
from pacer_worker.lib.models import Model, Run, Site
...
class RunManager(LoggerMixinBase):
    ...
    def create_model(self, file_path: os.PathLike) -> Model:
        ...
        if file_path.is_dir():
            ...
            file_path = Path(
                shutil.make_archive(temp_dir / file_path.stem, "zip", file_path)
            )
    ...
    def create_run_from_model(
        ...
        run = Run(
            dir=run_path, model=model, ref_id=run_id, sim_type=sim_type, name=model_name
        )
        run.save()
