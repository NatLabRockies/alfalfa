...
from pacer_worker.lib.enums import RunStatus, SimType
...
from pacer_worker.lib.redis_log_handler import RedisLogHandler
    ...
    def __new__(cls, name, bases, cls_dicts):
        ...
        def __new_init__(self: "Job", *args, **kwargs):
            if (
                ...
                and self._status.value >= JobStatus.INITIALIZING.value
            ):
                if __old_init__:
...
class Job(metaclass=JobMetaclass):
        ...
        try:
            if message and message["data"].__class__ == bytes:
                ...
                if method in self._message_handlers.keys():
                    ...
                    response = {}
                    try:
                        response["response"] = self._message_handlers[data["method"]](
                            **data.get("params", {})
                        )
                        response["status"] = "ok"
                        ...
                        self.redis.hset(
                            self.run.ref_id, message_id, json.dumps(response)
                        )
                        self.logger.info(
    ...
    def create_run_from_model(
        self, model_id: str, sim_type=SimType.OPENSTUDIO, run_id=None
    ) -> None:
    ...
    def log_subprocess(self, args: list[str], timeout: int = 300):
        ...
        if time() > wait_until:
            process.kill()
            exception = JobExceptionTimeout(
                f"Process timedout after: {timeout} seconds"
            )
            exception.add_note(process_log)
            raise exception
        exitcode = process.returncode
        if exitcode != 0:
            exception = JobExceptionExternalProcess(
                f"Process returned non-zero exit code: {exitcode}"
            )
            exception.add_note("\n".join(process_log))
            raise exception
