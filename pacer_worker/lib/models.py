...
from pacer_worker.lib.enums import PointType, RunStatus, SimType
...
class Point(TimestampedDocument):
    ...
    def value(self, value):
        ...
        self.redis.hset(
            self.redis_key + ":out",
            mapping={"curStatus": "s:ok", "curVal": f"n:{value}"},
        )
...
class Run(TimestampedDocument):
    ...
    def input_points(self) -> List[Point]:
        return Point.objects(
            Q(point_type=PointType.INPUT) | Q(point_type=PointType.BIDIRECTIONAL),
            run=self,
        )
    ...
    def output_points(self) -> List[Point]:
        return Point.objects(
            Q(point_type=PointType.OUTPUT) | Q(point_type=PointType.BIDIRECTIONAL),
            run=self,
        )
    ...
    def sim_time(self):
        ...
        if redis_entry:
            return datetime.datetime.strptime(
                redis_entry.decode("UTF-8"), "%Y-%m-%d %H:%M:%S"
            )
        else:
            return None
