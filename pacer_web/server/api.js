...
class PACERAPI {
  constructor(db, redis) {
    ...
    this.redisJobQueue = process.env.JOB_QUEUE || "PACER Job Queue";
  ...
  startRun = async (run, data) => {
    ...
    const job = `pacer_worker.jobs.${sim_type === "MODELICA" ? "modelica" : "openstudio"}.step_run.StepRun`;
    const params = {
      run_id: run.ref_id,
