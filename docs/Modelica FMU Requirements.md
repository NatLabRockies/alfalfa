# Uploading Modelica / FMU models to PACER

PACER can run Functional Mock-up Units (FMUs) through its **modelica** job path.
...
## How PACER handles an FMU

1. **Upload / submit.** A file whose name ends in `.fmu` is routed to the
   modelica job by extension. `pacer_worker/jobs/modelica/create_run.py` simply
...
2. **Run start.** When the run is started,
   `pacer_worker/jobs/modelica/step_run.py` → `initialize_simulation()` loads the
   FMU with `pyfmi.load_fmu` (via `pacer_worker/lib/testcase.py::TestCase`) and
   `setup_points()` creates PACER `Point` records:
...
An FMU must satisfy **all** of the following to run in PACER:

| Requirement | Detail |
|---|---|
| **FMI version** | `2.0`. PACER rejects other versions (`TestCase` raises `FMU must be version 2.0.`). |
...
- **`resources/kpis.json`** (inside the FMU zip). BOPTEST-wrapped FMUs embed a
  ...
  data). PACER now **tolerates its absence** (it proceeds with an empty KPI
...
- **Units / min / max.** BOPTEST FMUs declare a `unit` (and often min/max) on
  every I/O variable. PACER now **tolerates missing** unit/min/max/description
...
`u`; outputs `Q_flow`, `T_out`). These are valid and, with current PACER, run
...
2. **Provide meaningful inputs / start values.** These are single-component
   ...
   (`T_in`, `m_flow`, `u`, …) with realistic values via the PACER client.
