...
> like so it uploads **and runs** in PACER. It is self-contained and can be
...
An FMU is PACER-compatible when **all** of these are true:

- [ ] **FMI version 2.0** (not 1.0, not 3.0).
- [ ] **Co-Simulation** kind (the FMU embeds a `<CoSimulation>` element with binaries).
- [ ] **`linux64` binaries present** — `binaries/linux64/<model>.so` inside the FMU zip. PACER runs the FMU in a Linux x86-64 container; macOS/Windows-only binaries will **not** load.
- [ ] At least one variable with causality **`input`** and one with causality **`output`** (these become PACER points).
...
- [ ] Only declare **`min`/`max`** on an input if you actually want PACER to clamp it (see below).

---

## How PACER runs an FMU (so the contract makes sense)

1. **Upload.** A file ending in `.fmu` is accepted and stored; PACER renames it to
   `model.fmu`. **No validation happens at upload** — a successful upload does *not*
   mean the model will run.
2. **Start.** PACER loads the FMU (via `pyfmi.load_fmu`) and reads its model
...
3. **Advance.** On each step PACER writes the current input values into the FMU,
...
|---|---|---|
| **FMI version** | `2.0` | PACER rejects anything else (`FMU must be version 2.0.`). |
| **FMI kind** | **Co-Simulation** | PACER steps the model itself; a Model-Exchange-only FMU has no solver to step. |
...
## Variable naming & causality → PACER points

PACER supports **two** styles. Pick one per model.
...
PACER creates a single INPUT point named `<name>_u`. The `_activate` flag is
**managed automatically**: when you set a value on the `_u` point PACER writes
...
`resources/*.csv` forecast/price/weather data). PACER **tolerates its absence** —
...
- **Units / description are optional.** PACER tolerates variables with no declared
...
- **`min`/`max` are enforced when present.** If you declare `min`/`max` on an
  input, PACER **clamps** incoming values to that range. Only declare bounds you
...
| Symptom in PACER | Likely cause | Fix |
|---|---|---|
...
*Generated for PACER (formerly Alfalfa). Behavior described here reflects PACER's
modelica job path, which accepts both plain OpenModelica Co-Simulation FMUs and
BOPTEST-wrapped FMUs.*
